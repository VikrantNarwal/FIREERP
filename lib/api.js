...
class APIClient {
  ...
  // Parses the JSON body of a response and, if the response was not 2xx,
  // throws an Error carrying the backend's message instead of silently
  // returning the error payload as if it were a successful result.
  // (This is what let "Add Component" / "Save Changes" show a success toast
  // while the database write had actually failed and the row never existed.)
  ...
  // Pre-Assembly Variables (Design-managed measurement field definitions)
  async getPreAssemblyVariables() {
    return this.get('/design/pre-assembly-variables')
  }

  async createPreAssemblyVariable(data) {
    return this.post('/design/pre-assembly-variables', data)
  }

  async updatePreAssemblyVariable(id, data) {
    return this.put(`/design/pre-assembly-variables/${id}`, data)
  }

  async deletePreAssemblyVariable(id) {
    return this.delete(`/design/pre-assembly-variables/${id}`)
  }
